...
from modules.DataBase import get_all_users, get_statistics_by_month, get_statistics_by_date
from modules.calendar_kb import generate_calendar_kb, control_months_data, day_on_month_data, calendar_info_data, \
    get_month_text


@dp.callback_query_handler(statistics_main_data.filter(prefix='general'))
...
@dp.callback_query_handler(text='statistics_back')
...
async def process_get_next_month(call: types.CallbackQuery, callback_data: dict):
    ...
    year = int(callback_data.get("year"))
    date_calendar = datetime(year=int(year), month=int(month), day=1)
    now = datetime.now(UTC_TIME_ZONE)
    if date_calendar > now:
        await call.answer(text="Этот месяц еще не наступил!")
        return
    users = len(get_all_users())
    text = f"Статистика за {get_month_text(int(month))} {year}\n\nВсего пользователей {users}\n"
    statistic = get_statistics_by_month(int(month), int(year))
    text += statistic
...
async def process_get_next_month(call: types.CallbackQuery, callback_data: dict):
    ...
    if date_calendar > now:
        ...
        return
    users = len(get_all_users())
    text = f"Статистика за {day}.{month}.{year}\n\nВсего пользователей {users}\n"
    statistic = get_statistics_by_date(int(day), int(month), int(year))
    text += statistic
    calendar = generate_calendar_kb(year=year, month=month)
    await call.message.edit_text(text, reply_markup=calendar)