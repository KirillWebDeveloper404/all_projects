...
from keyboard.inline.edit.edit_video_note import generate_get_video_note_keyboard, edit_video_note_msg_data
from loader import dp
from states import EditMessages
from utils.functions.edit_messages_msg import get_text_edit_messages


@dp.callback_query_handler(edit_message_kb_data.filter(prefix='video_note'), state=EditMessages.start)
async def process_start_edit(call: types.CallbackQuery, state: FSMContext):
    ...
    if data['video_note']:
        ...
        await EditMessages.video_note.set()
    await call.message.answer(text, reply_markup=keyboard)


@dp.message_handler(state=EditMessages.video_note, content_types=types.ContentTypes.VIDEO_NOTE)
...
@dp.callback_query_handler(edit_video_note_msg_data.filter(prefix='change'), state=EditMessages.start)
...
@dp.message_handler(state=EditMessages.video_note, content_types=types.ContentTypes.ANY)
