...
from keyboard.inline.manage_message.all_system_messages import get_all_system_messages_kb, system_message
from keyboard.inline.manage_message.manage_message import get_manage_messages_keyboard, manage_message_af_kb_data
from loader import dp
from modules.DataBase import get_message_af_by_id, delete_message_af_by_id
from utils.functions.send_manage_message_funnel import send_manage_message_funnel


@dp.callback_query_handler(auto_funnels_manage_messages_data.filter(prefix='system'))
async def process_get_content_messages(call: types.CallbackQuery, callback_data: dict):
    ...
    keyboard = await get_all_system_messages_kb(funnel_id)
    await call.message.answer('Выберете сообщение', reply_markup=keyboard)


@dp.callback_query_handler(system_message.filter(message_id='None'))
...
@dp.callback_query_handler(system_message.filter())
async def process_back(call: types.CallbackQuery, callback_data: dict):
    await call.message.delete()
    message_id = callback_data.get('message_id')
    await call.answer(cache_time=1)
    keyboard = await get_manage_messages_keyboard(message_id, 'system')
    await send_manage_message_funnel(message_id, call.from_user.id, keyboard=keyboard)


@dp.callback_query_handler(manage_message_af_kb_data.filter(prefix='system_back'))
async def process_back(call: types.CallbackQuery, callback_data:dict):
...
@dp.callback_query_handler(manage_message_af_kb_data.filter(prefix='delete'))
async def process_back(call: types.CallbackQuery, callback_data: dict):
    message_id = int(callback_data.get('message_id'))
    message_auto = get_message_af_by_id(message_id)

    delete_message_af_by_id(message_id)
    await call.answer(cache_time=1)
    await call.message.delete()
    keyboard = await manage_messages(message_auto.auto_funnel_id)
    await call.message.answer('Управление сообщениями в воронке', reply_markup=keyboard)
