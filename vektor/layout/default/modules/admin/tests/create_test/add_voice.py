...
@dp.callback_query_handler(create_message_result.filter(prefix='voice'), state=AddTest.create_result)
async def process_get(call: types.CallbackQuery, state: FSMContext):
    await call.answer()
    data = await state.get_data()
    keyboard = await get_voice_result(data)
    text = 'Отправьте голосовое сообщение'
    if data['voice']:
        text = 'Выберете'
    else:
        await AddTest.get_voice.set()
    await call.message.delete()
    await call.message.answer(text, reply_markup=keyboard)


@dp.callback_query_handler(get_result_voice_dt.filter(prefix='change'), state=AddTest.create_result)
...
@dp.message_handler(state=AddTest.get_voice, content_types=types.ContentTypes.VOICE)
